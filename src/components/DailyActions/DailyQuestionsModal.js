...
export const DailyQuestionsModal = ({ visible, onClose }) => {
  const { logTodayEffort, userId, username, currentWeek, fetchDashboardData } =
    useUser();
  ...
  const fetchQuestions = async () => {
    ...
    try {
      const response = await fetch(
        `https://sbm-mobile-app-906714478.development.catalystserverless.com/tracker/questions?userId=${userId}&type=questions`,
      );
  ...
  const handleSubmitDailyLog = async () => {
    ...
    if (unanswered.length > 0) {
      alert(
        `Please answer all questions before submitting. Unanswered: ${unanswered.map((_, idx) => idx + 1).join(", ")}`,
      );
      return;
    }
    ...
    try {
      ...
      console.error(err);
      alert(
        "Network Error: Could not connect to Catalyst to submit daily log.",
      );
    }
  };
  ...
  const handleSubmitSundayLog = async () => {
    ...
    try {
      ...
      try {
        await fetch(
          "https://sbm-mobile-app-906714478.development.catalystserverless.com/api/sunday_tracker/submit",
          {
            method: "POST",
        ...
        console.warn(
          "Sunday tracker submit offline notice (saving locally):",
          e,
        );
      }
                ...
                >
                  ...
                  <Text
                    style={[
                      ...
                      styles.reviewOptionTextChecked,
                      {
                        flex: 1,
                        textAlign: "left",
                        marginLeft: 8,
  ...
  const renderSundayIntroView = () => {
    return (
      <View style={{ alignItems: "center", paddingVertical: 20 }}>
        ...
        >
          Great job logging your daily effort! Since today is{" "}
          <Text style={{ color: "#B085F5", fontWeight: "700" }}>Sunday</Text>,
          take 1 minute to answer 5 quick questions about your weekly mindset.
        </Text>
  ...
  const renderSundayQuestionView = () => {
    ...
    return (
      <View>
        <View style={styles.modalHeader}>
          <Text style={styles.modalTitle}>
            Sunday Mindset ({sundayIndex + 1}/5)
          </Text>
          <TouchableOpacity style={styles.closeBtn} onPress={handleCloseAll}>
            <X size={20} color="#FFFFFF" />
          </TouchableOpacity>
        ...
        <View style={{ marginTop: 16 }}>
          ...
          >
            ...
            >
              ...
              >
                {sundayIndex === SUNDAY_QUESTIONS.length - 1
                  ? "Complete & Save Sunday Check-in"
                  : "Next Mindset Question →"}
              </Text>
            </TouchableOpacity>
          </LinearGradient>
        </View>
  ...
  return (
    ...
    >
      <View
        style={[styles.modalOverlay, isWebDesktop && styles.webModalOverlay]}
      >
